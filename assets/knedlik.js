/* 
	...
	Founded 23.04.2014, released 29.04.2014 (v.1), updated 14.01.2015, then 27.01.2016
...
function knedlik(elem, prefs) {
	...
	var process = function(el) {
		...
		var colorsDef = { 'background': '#eee', 'backgroundBtn': '#eee', 'backgroundActive': '#ddd', 'border': '#ddd', 'borderBtn': '#ccc', 'borderActive': '#aaa', 'shadowBtn': '#ddd', 'shadowActive': '#bbb', 'shadowColor': '#bbb', 'colorBtn': '#444', 'colorClose': '#bbb', 'colorize': '#070', 'colorizeA': '#00c', 'colorizeH': '#dc143c' }
		if(prefs.colors) {
			for(clr in colorsDef) {
				if(prefs.colors[clr]) colorsDef[clr] = prefs.colors[clr];
				}
			}
		...
		css(el, {border:0, width:0, height:0, overflow:'hidden', opacity:0, position:'absolute', left:'-9999px', top:'-9999px'});
		
		var editor = document.createElement('div');
		editor.id = 'knedlik-editor-'+knedlikEditorNb+'-area';
		el.parentNode.insertBefore(editor, el);
		css(editor, {border:'1px solid '+colorsDef.border, width:(parseInt(elw)+'px'), height:(parseInt(elh)+'px'), overflow:'auto', padding:'7px', margin:0, minWidth:'450px', minHeight:'175px'});
		html(editor, html(el).replace(/&lt;/g,'<').replace(/&gt;/g,'>'));
		...
		css(panel, {border:'1px solid '+colorsDef.border, borderBottom:0, background:colorsDef.background, width:(parseInt(elw)+'px'), padding:'7px', margin:0, minWidth:'450px'});
		...
		var closeDropdown = function(id) {
			...
			css(_('knedlik-editor-'+knedlikEditorNb+'-button-'+id), {background:colorsDef.backgroundBtn});
		...
		var colorize = function(id) {
			...
			for(ci in colors) {
				ret += '<div style="width:16px;height:16px;float:left;margin:2px;overflow:hidden;cursor:pointer;background-color:#'+colors[ci][2]+';box-shadow:0 0 2px '+colorsDef.shadowColor+';border-radius:2px;" title="'+colors[ci][langIndex]+'" data-color="'+colors[ci][2]+'">&nbsp;</div>';
				}
		...
		var dialogs = {
			'font': {
				'form': function() {
					...
					fontsSelect += '</select>';
					var form = fontsSelect+' <input id="knedlik-forms-font-size" type="text" style="width:25px;">pt <input type="button" id="knedlik-forms-font-submit" value="'+kTranslations.ok+'">';
					return form;
					},
				'responder': function() {
			...
			'link': {
				'form': function() {
					var form = '<input type="text" id="knedlik-forms-link-text" placeholder="URL"> <input type="button" id="knedlik-forms-link-submit" value="'+kTranslations.ok+'"><br><input type="checkbox" id="knedlik-forms-link-check" style="display:inline;"> <label for="knedlik-forms-link-check" style="display:inline;">'+kTranslations.newwin+'</label><div style="padding-top:5px;"><a href="" id="knedlik-remove-link">'+kTranslations.unlink+'</a></div>';
					return form;
					},
				'responder': function() {
			...
			'diacritic': {
				'form': function() {
					...
					var diacrs = '';
					for(d in diacr) {
						diacrs += (diacr[d]=='|') ? '<br clear="all">' : '<div style="display:block; float:left; height:20px; line-height:20px; overflow:hidden; padding:0 5px; margin:0 4px 4px 0; border-radius:1px; box-shadow:0 0 3px '+colorsDef.shadowBtn+'; cursor:pointer; color:'+colorsDef.colorBtn+'; font-family:Courier New, monospace; font-size:10.5pt;" title="'+diacr[d]+'">'+diacr[d]+'</div>';
						}
			...
			'html': {
				'form': function() {
					var form = '<textarea id="knedlik-forms-html-text" cols="30" rows="15">'+html(editor)+'</textarea><br><input type="button" id="knedlik-forms-html-submit" value="'+kTranslations.ok+'">';
					return form;
					},
				'responder': function() {
			...
			'plushtml': {
				'form': function() {
					var form = '<textarea id="knedlik-forms-plushtml-text" cols="30" rows="15"></textarea><br><input type="button" id="knedlik-forms-plushtml-submit" value="'+kTranslations.ok+'">';
					return form;
					},
				'responder': function() {
		...
		var buttons = {
			'format': { 'name': kTranslations.formatName, 'dropdown': true },
				...
				'sub': { 'name': 'x<sub style="font-size:6pt; color:'+colorsDef.colorizeH+'">2</sub>', 'cmd': 'SubScript', 'inside': 'format' },
				'sup': { 'name': '<span style="position:relative;top:-3px;">x<sup style="font-size:6pt; color:'+colorsDef.colorizeH+'">2</sup></span>', 'cmd': 'SuperScript', 'inside': 'format' },
			...
			'align': { 'name': '<img src="data:image/gif;base64,R0lGODlhFgAWAID/AMDAwAAAACH5BAEAAAAALAAAAAAWABYAQAIghI+py+0Po5y02ouz3jL4D4JMGELkGYxo+qzl4nKyXAAAOw==" style="margin-top:1px;">', 'cmd': 'JustifyLeft', 'dropdown': true },
				'align-left': { 'name': '<img src="data:image/gif;base64,R0lGODlhFgAWAID/AMDAwAAAACH5BAEAAAAALAAAAAAWABYAQAIghI+py+0Po5y02ouz3jL4D4JMGELkGYxo+qzl4nKyXAAAOw==" style="margin-top:1px;">', 'cmd': 'JustifyLeft', 'inside': 'align' },
				'align-center': { 'name': '<img src="data:image/gif;base64,R0lGODlhFgAWAID/AMDAwAAAACH5BAEAAAAALAAAAAAWABYAQAIfhI+py+0Po5y02ouz3jL4D4JOGI7kaZ5Bqn4sycVbAQA7" style="margin-top:1px;">', 'cmd': 'JustifyCenter', 'inside': 'align' },
				'align-right': { 'name': '<img src="data:image/gif;base64,R0lGODlhFgAWAID/AMDAwAAAACH5BAEAAAAALAAAAAAWABYAQAIghI+py+0Po5y02ouz3jL4D4JQGDLkGYxouqzl43JyVgAAOw==" style="margin-top:1px;">', 'cmd': 'JustifyRight', 'inside': 'align' },

			'link': { 'name': 'A', 'css': {color:colorsDef.colorizeA, textDecoration:'underline'}, 'dropdown': true },
			
			'image': { 'name': '<img src="data:image/gif;base64," style="margin-top:1px;">', 'dropdown': true },
			
			'lists': { 'name': '<img src="data:image/gif;base64,R0lGODlhFgAWAMIGAAAAAB1ChF9vj1iE33mOrqezxv///////yH5BAEAAAcALAAAAAAWABYAAAMyeLrc/jDKSesppNhGRlBAKIZRERBbqm6YtnbfMY7lud64UwiuKnigGQliQuWOyKQykgAAOw==" style="margin-top:1px;">', 'dropdown': true },
				'ul': { 'name': '<img src="data:image/gif;base64,R0lGODlhFgAWAMIGAAAAAB1ChF9vj1iE33mOrqezxv///////yH5BAEAAAcALAAAAAAWABYAAAMyeLrc/jDKSesppNhGRlBAKIZRERBbqm6YtnbfMY7lud64UwiuKnigGQliQuWOyKQykgAAOw==" style="margin-top:1px;">', 'cmd': 'InsertUnorderedList', 'inside': 'lists' },
				'ol': { 'name': '<img src="data:image/gif;base64,R0lGODlhFgAWAMIGAAAAADljwliE35GjuaezxtHa7P///////yH5BAEAAAcALAAAAAAWABYAAAM2eLrc/jDKSespwjoRFvggCBUBoTFBeq6QIAysQnRHaEOzyaZ07Lu9lUBnC0UGQU1K52s6n5oEADs=" style="margin-top:1px;">', 'cmd': 'InsertOrderedList', 'inside': 'lists' },

			'plushtml': { 'name': '+&nbsp;HTML', 'css': {fontSize:'8pt'}, 'dropdown': true },
			
			'fgcolor': { 'name': kTranslations.fgName, 'css': {color:colorsDef.colorize}, 'dropdown': true },
			'bgcolor': { 'name': kTranslations.bgName, 'css': {color:colorsDef.colorize}, 'dropdown': true },

			'table': { 'name': '<img src="data:image/gif;base64,R0lGODlhFgAWAJECAKa7zqWlpdjY7AAAACH5BAEAAAIALAAAAAAWABYAAAI8lI+py+0PY5i02qAC2Lx3nGjeuIHIhVLZyR6m2xrvK8fCvMJ6HaZozgvegMNd0YYzCpNCme8SiUqn1GkBADs=" style="margin-top:1px;">', 'dropdown': true },
			...
			'html': { 'name': 'HTML', 'css': {fontSize:'8pt', color:colorsDef.colorizeH}, 'dropdown': true }
			}
		var buttonsPassed = {};
		if(prefs.buttons) {
		...
		for(ij in buttonsPassed) {
			...
			if(button.name=='|') {
				...
				where.appendChild(addButton);
				css(addButton, { border: '1px solid '+colorsDef.borderBtn, height:'25px', lineHeight:'23px', padding:'0 4px', textAlign:'center', color:colorsDef.colorBtn, fontSize:'8pt', fontFamily:'Tahoma, Sans-Serif', cursor:'pointer', margin:'1px', cssFloat:'left', boxShadow:'0 0 2px '+colorsDef.shadowBtn, borderRadius:'3px', background:colorsDef.backgroundBtn });
				...
				if(button.dropdown) {
					...
					css(panelDropdown, {position:'absolute', left:(parseInt(buttonLeft)-1)+'px', top:(parseInt(buttonTop)+29)+'px', background:colorsDef.backgroundBtn, marginLeft:'1px', border: '1px solid '+colorsDef.borderBtn, padding:'9px', paddingTop:'25px', textAlign:'left', color:colorsDef.colorBtn, fontSize:'9pt', fontFamily:'Sans-Serif', boxShadow:'0 0 2px '+colorsDef.shadowBtn, borderRadius:'3px', display:'none', minWidth:'150px' });
				...
				addButton.onmouseover = function() { 
					css(this, {border:'1px solid '+colorsDef.borderActive, boxShadow:'0 0 2px '+colorsDef.shadowActive}); 
					if(prefs.style && prefs.style=='flat') { css(this, { boxShadow:'none' }); }
					}
				addButton.onmouseout = function() { 
					css(this, {border:'1px solid '+colorsDef.borderBtn, boxShadow:'0 0 2px '+colorsDef.shadowBtn});
					if(prefs.style && prefs.style=='flat') { css(this, { boxShadow:'none' }); }
					}
				addButton.onmousedown = function(event) {
					...
					if(_('knedlik-editor-'+knedlikEditorNb+'-panel-dropdown-'+rel)) {
						...
						if(disp=='block') {
							css(_this, {background:colorsDef.backgroundActive});
							...
							css(addCloser, {position:'absolute', top:'3px', right:'5px', color:colorsDef.colorClose, cursor:'pointer', fontSize:'12pt'});
							...
							css(_this, {background:colorsDef.backgroundBtn});
							}
						}
					else {
